...
| Component | Plugin/Config | What It Does | Required Config |
|-----------|--------------|-------------|----------------|
| **LanceDB vector store** | `@openclaw/memory-lancedb` | Semantic vector search with embeddings | `plugins.slots.memory = "memory-lancedb"` |
| **Embedding model** | `agents.defaults.memorySearch` | Generates vectors from text | `provider: "ollama"`, `model: "nomic-embed-text:v1.5"`, `dimensions: 768` |
| **Hybrid search** | `memorySearch.query.hybrid` | Blends vector + keyword results | `vectorWeight: 0.7`, `textWeight: 0.3` |
| **MMR dedup** | `memorySearch.query.hybrid.mmr` | Removes redundant results | `enabled: true`, `lambda: 0.7` |
| **Temporal decay** | `memorySearch.query.hybrid.temporalDecay` | Recent memories rank higher | `enabled: true`, `halfLifeDays: 30` |
| **Auto-recall** | `memory-lancedb config` | Injects relevant memories before model turns | `autoRecall: true` |
| **Auto-capture** | `memory-lancedb config` | Stores important facts after responses | `autoCapture: true` |
| **Active Memory** | `active-memory` plugin | Interactive recall sub-agent | `enabled: true`, `hooks.allowConversationAccess: true` |
| **Dreaming** | `memory-core` plugin | Nightly consolidation of short-term → long-term | `enabled: true`, `config.dreaming.enabled: true` |
| **Memory Wiki** | `memory-wiki` plugin | Obsidian-style knowledge vault | `enabled: true` |
| **Session memory** | `memorySearch.experimental` | Searches session history too | `sessionMemory: true` |

### Verification Checklist

Run this on EVERY machine before considering Context Bridge operational:

```bash
# 1. LanceDB installed and owns the memory slot
openclaw plugins list | grep memory-lancedb
# Must show: enabled

# 2. Memory slot is LanceDB (not memory-core)
openclaw status | grep Memory
# Must show: "enabled (plugin memory-lancedb)"
# NOT: "enabled (plugin memory-core)" or "unknown"

# 3. Vector store is active (not "unknown")
openclaw ltm stats
# Must show: Total memories >= 0 (starts at 0, grows with use)
# If command fails, LanceDB is not loaded

# 4. Embedding model reachable
curl -s http://<ollama-host>:11434/v1/embeddings \
  -H "Content-Type: application/json" \
  -d '{"model":"nomic-embed-text:v1.5","input":"test"}' | jq '.data[0].embedding | length'
# Must return: 768

# 5. Hybrid search configured
openclaw config get agents.defaults.memorySearch.query.hybrid
# Must return object with vectorWeight, textWeight, mmr, temporalDecay
```
...
# See "Reference Config" section below for the full block
```

### ⚠️ Common Misconfiguration

**"memorySearch.provider: ollama" is NOT enough.**

Having `memorySearch` pointing at an Ollama embed model generates embeddings, but without LanceDB those embeddings go into a SQLite cache — NOT a vector index. The status will show `Vector store: unknown` and search degrades to BM25 keyword matching only. You MUST install `memory-lancedb` and set `plugins.slots.memory = "memory-lancedb"` for actual semantic search.

### Reference Config

The minimum `openclaw.json` additions for full semantic memory:
    ...
    "entries": {
      ...
      "memory-core": {
        ...
        "subagent": {
          ...
          "allowedModels": ["ollama/glm-5.1:cloud"]
        },
        "config": {
          "dreaming": {
            "enabled": true,
...
**Note:** The `memory-core` plugin will show a warning "plugin disabled (memory slot set to \"memory-lancedb\") but config is present" — this is **cosmetic and expected**. Memory-core still runs dreaming and other features; it just doesn't own the memory slot anymore. LanceDB owns that slot.

## Important: After OpenClaw Upgrades

**CRITICAL:** OpenClaw upgrades can disconnect workspace skills from the skill registry. If context-bridge stops working after an upgrade:

```bash
# Check if installed
openclaw skills list | grep context-bridge

# If missing, reinstall:
openclaw skills install context-bridge --force
```

**Symptoms of broken context-bridge:**
- `model-handoff.md` is stale (last updated before upgrade)
- `skills-discovery.json` shows old timestamps
- Model sessions "start from zero" — no cross-session context
- Agent says "I don't know about that" for previously discussed topics

**Root cause:** Workspace skills exist on disk but aren't registered in OpenClaw's skill system. Upgrades rebuild the registry and drop workspace skills.

**Prevention:** Set up auto-verification (see Auto-Update section below).
...
## Auto-Update Setup (Prevents Skill Loss)

### Linux (systemd/cron)

Create a cron job that verifies and updates context hourly:

```bash
# Add to crontab (crontab -e)
0 * * * * /home/user/.openclaw/scripts/update-context-bridge.sh
```
...
```bash
#!/bin/bash
# ~/.openclaw/scripts/update-context-bridge.sh
HANDOFF_FILE="$HOME/.openclaw/model-agnostic-memory/model-handoff.md"
DATE=$(date -u +"%Y-%m-%dT%H:%M:%SZ")

# Get last session info from memory files
LATEST_MEM=$(ls -t $HOME/.openclaw/workspace/memory/2026-*.md 2>/dev/null | head -1)
MEM_DATE=""
if [ -n "$LATEST_MEM" ]; then
    MEM_DATE=$(basename "$LATEST_MEM" | sed 's/.md//')
fi

# Update the handoff file
cat > "$HANDOFF_FILE" << ENDFILE
# Model Handoff Log
...
**Updated:** $DATE
**Latest Memory File:** ${MEM_DATE:-none}

## Important: After OpenClaw Upgrades

Verify skills: openclaw skills list | grep context-bridge
Reinstall: openclaw skills install context-bridge --force
ENDFILE

echo "Context bridge updated at $DATE"
```
...
```bash
# Save as ~/Library/LaunchAgents/com.openclaw.context-bridge-update.plist
cat > ~/Library/LaunchAgents/com.openclaw.context-bridge-update.plist << 'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.openclaw.context-bridge-update</string>
    <key>ProgramArguments</key>
    <array>
        <string>/bin/bash</string>
        <string>/Users/username/.openclaw/scripts/update-context-bridge.sh</string>
    </array>
    ...
    <string>/tmp/com.openclaw.context-bridge-update.stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/com.openclaw.context-bridge-update.stderr.log</string>
</dict>
</plist>
EOF

# Load it
launchctl load ~/Library/LaunchAgents/com.openclaw.context-bridge-update.plist
```

### Using generate_plist.py (Fleet Tool)

For fleet deployments, use the included script:

```bash
python3 generate_plist.py \
  --label com.openclaw.context-bridge-update \
  --interval 3600 \
  --script /Users/username/.openclaw/scripts/update-context-bridge.sh \
  --machine cosmos
```
